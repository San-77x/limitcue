...
export default function Home() {
  return <main>
    <nav className="nav container"><a href="#top" className="brand"><span className="brand-mark"><i /><i /><i /></span><span>limitcue</span></a><div className="nav-links"><a href="#features">Features</a><a href="#how">How it works</a><a href="#preview">What&apos;s included</a></div><a className="button button-small" href={RELEASES}>Download for Linux <span>↓</span></a></nav>

    <section className="hero container" id="top"><div className="hero-copy"><div className="eyebrow"><span className="eyebrow-line" /> NATIVE LINUX QUOTA MONITOR</div><h1>Know your limits<br /><em>before they interrupt</em> your flow.</h1><p className="hero-lede">One quiet rail for every AI coding plan you use. See what is left, when it resets, and whether the number is real — without opening another browser tab.</p><div className="hero-actions"><a className="button" href={RELEASES}>Download for Linux <span>↓</span></a><a className="text-link" href="#preview">See what is included <span>↓</span></a></div><div className="proof-row"><span>Rust + egui</span><span>Wayland + X11</span><span>No telemetry</span><span>Public binary · source coming soon</span></div></div><ProductMockup /></section>
    ...
    <section className="providers container" id="preview"><div><span className="section-kicker">WORKS WITH YOUR STACK</span><h2>One surface.<br /><em>Many providers.</em></h2></div><div className="provider-grid">{providers.map(([id, name]) => <div className="provider-tile" key={id}><Logo id={id} /><span>{name}</span></div>)}<div className="provider-tile custom"><span className="plus">＋</span><span>Custom JSON</span></div></div></section>
    ...
    <section className="final-cta container"><span className="section-kicker">AVAILABLE FOR TESTING</span><h2>Keep your context.<br /><em>Lose the quota anxiety.</em></h2><p>Download the native Linux monitor and help shape the product while the public source release is being prepared.</p><a className="button" href={RELEASES}>Download for Linux <span>↓</span></a><small>Public binary · open-source release coming soon</small></section>

    <footer className="footer container"><a href="#top" className="brand"><span className="brand-mark"><i /><i /><i /></span><span>limitcue</span></a><span>Ambient quota visibility for Linux.</span><div><a href="#privacy">Privacy</a><a href="#top">Back to top ↑</a></div></footer>
  </main>;
}
